...
const validatorType2 = new types.ContainerType<Validator>({
  fields: {
    pubkey: bytes48Type,
...
suite
  // @ts-ignore
  .add("iterate - structural - 10k", () => {
    for (const v of struct10k) {
    }
  })
  // @ts-ignore
  .add("iterate - tree - 10k", () => {
    for (const v of tree10k) {
    }
  })
  //
  ...
  //
  .add(
    "hashTreeRoot - structural - 10k",
    () => updateValidator(struct10k) && validatorRegistryType2.hashTreeRoot(struct10k)
  )
